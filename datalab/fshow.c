...
/* Get exponent */
unsigned get_exp(unsigned uf) { return (uf >> FRAC_SIZE) & EXP_MASK; }

/* Get fraction */
unsigned get_frac(unsigned uf) { return uf & FRAC_MASK; }

/* Get sign */
unsigned get_sign(unsigned uf) { return (uf >> (FLOAT_SIZE - 1)) & 0x1; }
...
void show_float(unsigned uf) {
  ...
  printf("Bit Representation 0x%.8x, sign = %x, exponent = 0x%.2x, fraction = "
         "0x%.6x\n",
         uf, sign, exp, frac);
  if (exp == EXP_MASK) {
    if (frac == 0) {
